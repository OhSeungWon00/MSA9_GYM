...
- 랭킹 페이지의 경우 나의 랭킹으로 이동 버튼을 통해서 나의 출석랭킹에 바로 도달 할 수 있지만 해당 기능을 사용하려면 랭킹 목록에 페이 
  징 기능을 사용할 수 없고 유저 수 만큼 페이지 스크롤이 길어진다. 페이징 기능을 사용하면서도 나의 랭킹으로 이동 버튼을 구현할 수 있게 
  하는 방법을 찾거나 차라리 나의 랭킹으로 이동 버튼 대신 나의 랭킹이 몇등인지 바로 조회할 수 있도록 변경을 할지 구조를 고민해봐야 할 
  것 같다.
